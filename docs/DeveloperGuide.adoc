...
****
* Hints
** Just like we store each individual command word constant `COMMAND_WORD` inside `*Command.java` (e.g.  link:{repoURL}/src/main/java/seedu/address/logic/commands/FindCommand.java[`FindCommand#COMMAND_WORD`], link:{repoURL}/src/main/java/seedu/address/logic/commands/MemberDeleteCommand.java[`MemberDeleteCommand#COMMAND_WORD`]), you need a new constant for aliases as well (e.g. `FindCommand#COMMAND_ALIAS`).
...
|`* * *` |CCA main committee member |see which members are interested in attending a future event |decide whether to execute the activity or to cancel it based on the level of  interests.
...
(For all use cases below, the *System* is the `AddressBook` and the *Admin* is the `user`, unless specified otherwise)
...
[discrete]
=== Use case: Sort Club Manager

*Main Success Scenario (MSS)*

1.  Admin requests to sort the Club Manager.
2.  Club Manager sorts the members by name alphabetically.
